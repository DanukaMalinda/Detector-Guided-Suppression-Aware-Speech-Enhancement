...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, training_indexes, clean_wavs_dir, noisy_wavs_dir, segment_size, 
        ...
        # ✅ Pre-filter: keep only files that exist in both dirs
        valid_files = []
        for f in training_indexes:
            clean_path = os.path.join(self.clean_wavs_dir, f + ".wav")
            noisy_path = os.path.join(self.noisy_wavs_dir, f + ".wav")
            if os.path.exists(clean_path) and os.path.exists(noisy_path):
                valid_files.append(f)
            else:
                print(f"[WARNING] Skipping {f}: missing clean/noisy file")

        self.audio_indexes = valid_files
